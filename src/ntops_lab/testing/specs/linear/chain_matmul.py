import torch
from ntops_lab.kernels.linear.chain_matmul import run

OP_NAME = "chain_matmul"
IMPLEMENTATION_STATUS = "implemented_torch_top_batch1"

def make_inputs(device="cuda", dtype=torch.float32):
    return torch.randn((128, 64), device=device, dtype=torch.float16), torch.randn((64, 96), device=device, dtype=torch.float16), torch.randn((96, 32), device=device, dtype=torch.float16)
...
def run_pytorch(*inputs):
    ...
    return torch.linalg.multi_dot(list(inputs))
