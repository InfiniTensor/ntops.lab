...
from ntops_lab.kernels.fused.general.hinge_embedding_loss import run

OP_NAME = "hinge_embedding_loss"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"

def make_inputs(numel=1048576, device="cuda", dtype=torch.float32):
    x = torch.rand((numel,), device=device, dtype=dtype) * 2.0
    target = torch.where(torch.rand((numel,), device=device) > 0.5, 1.0, -1.0).to(dtype)
    return x, target

def run_pytorch(*inputs):
    x, target = inputs
    return F.hinge_embedding_loss(x, target, margin=1.0, reduction="none")
