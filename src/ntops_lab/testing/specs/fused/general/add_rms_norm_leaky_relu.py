import torch
from ntops_lab.kernels.fused.general.add_rms_norm_leaky_relu import run
...
ROWS = 2048
HAS_RESIDUAL = True
OP_NAME = "add_rms_norm_leaky_relu"
...
def run_pytorch(*inputs):
    ...
    value = (x + residual) * torch.rsqrt(torch.mean((x + residual) * (x + residual), dim=1, keepdim=True) + EPS) * weight
    return torch.nn.functional.leaky_relu(value, negative_slope=0.01)
