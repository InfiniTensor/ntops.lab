import torch
from ntops_lab.kernels.fused.general.rms_norm_leaky_relu import run
...
def run_pytorch(*inputs):
    x, weight = inputs
    value = x * torch.rsqrt(torch.mean((x) * (x), dim=1, keepdim=True) + EPS) * weight
