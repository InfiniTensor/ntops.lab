import torch
from ntops_lab.kernels.pointwise.atan2_scalar_left_out import run

OP_NAME = "atan2_scalar_left_out"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"
DEFAULT_NUMEL = 1048576

def make_inputs(numel=DEFAULT_NUMEL, device="cuda", dtype=torch.float32):
    x = torch.rand((numel,), device=device, dtype=dtype) * 4.0 + 0.25
    y = torch.rand((numel,), device=device, dtype=dtype) * 4.0 + 0.25
    return x, y

def run_pytorch(*inputs):
    x, y = inputs
    return torch.atan2(torch.full_like(x, 1.25), x)

def check(atol=5.0e-3, rtol=5.0e-3):
    inputs = make_inputs()
    actual = run(*inputs)
