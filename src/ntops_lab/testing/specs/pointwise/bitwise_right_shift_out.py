import torch
from ntops_lab.kernels.pointwise.bitwise_right_shift_out import run

OP_NAME = "bitwise_right_shift_out"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"
DEFAULT_NUMEL = 1048576

def make_inputs(numel=DEFAULT_NUMEL, device="cuda", dtype=torch.float32):
    x = torch.randint(0, 32, (numel,), device=device, dtype=torch.int32)
    y = torch.randint(0, 4, (numel,), device=device, dtype=torch.int32)
    return x, y
