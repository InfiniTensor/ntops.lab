import torch
from ntops_lab.kernels.pointwise.atan2_scalar_out import run

OP_NAME = "atan2_scalar_out"
...
def make_inputs(numel=DEFAULT_NUMEL, device="cuda", dtype=torch.float32):
    ...
    y = torch.randn((numel,), device=device, dtype=dtype) * 0.5
    return x, y

def run_pytorch(*inputs):
    x, y = inputs
    return torch.atan2(x, torch.full_like(x, 1.25))
