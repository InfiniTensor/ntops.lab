...
from ntops_lab.kernels.pointwise.dropout3d import run

OP_NAME = "dropout3d"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"

def make_inputs(device="cuda", dtype=torch.float32):
    return (torch.randn((2, 8, 16, 32, 32), device=device, dtype=dtype) * 0.5,)

def run_pytorch(*inputs):
    (x,) = inputs
    return F.dropout3d(x, p=0.5, training=False)
