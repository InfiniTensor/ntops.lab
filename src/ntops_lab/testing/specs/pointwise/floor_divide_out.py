import torch
from ntops_lab.kernels.pointwise.floor_divide_out import run

OP_NAME = "floor_divide_out"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"
DEFAULT_NUMEL = 1048576

def make_inputs(numel=DEFAULT_NUMEL, device="cuda", dtype=torch.float32):
    x = torch.rand((numel,), device=device, dtype=dtype) * 8.0 + 0.25
    y = torch.rand((numel,), device=device, dtype=dtype) * 4.0 + 0.5
    return x, y
