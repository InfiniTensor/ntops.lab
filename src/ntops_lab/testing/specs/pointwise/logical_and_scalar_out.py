import torch
from ntops_lab.kernels.pointwise.logical_and_scalar_out import run

OP_NAME = "logical_and_scalar_out"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"
DEFAULT_NUMEL = 1048576

def make_inputs(numel=DEFAULT_NUMEL, device="cuda", dtype=torch.float32):
    x = torch.randint(0, 2, (numel,), device=device, dtype=torch.int32)
    return (x,)

def run_pytorch(*inputs):
    x, = inputs
    return torch.logical_and(x, torch.ones_like(x, dtype=torch.bool))
