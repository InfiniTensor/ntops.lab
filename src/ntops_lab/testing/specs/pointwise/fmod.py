import torch
from ntops_lab.kernels.pointwise.fmod import run
...
def make_inputs(numel=DEFAULT_NUMEL, device="cuda", dtype=torch.float32):
    x = torch.randn((numel,), device=device, dtype=dtype) * 10.0
    y = torch.rand((numel,), device=device, dtype=dtype) * 3.0 + 0.5
    return x, y
