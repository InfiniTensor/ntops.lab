...
from ntops_lab.kernels.pointwise.dropout1d import run

OP_NAME = "dropout1d"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"

def make_inputs(device="cuda", dtype=torch.float32):
    return (torch.randn((64, 16, 1024), device=device, dtype=dtype) * 0.5,)

def run_pytorch(*inputs):
    (x,) = inputs
    return F.dropout1d(x, p=0.5, training=False)
