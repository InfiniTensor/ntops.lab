...
from ntops_lab.kernels.pointwise.feature_alpha_dropout import run

OP_NAME = "feature_alpha_dropout"
IMPLEMENTATION_STATUS = "implemented_ninetoothed"

def make_inputs(device="cuda", dtype=torch.float32):
    return (torch.randn((8, 16, 64, 64), device=device, dtype=dtype) * 0.5,)

def run_pytorch(*inputs):
    (x,) = inputs
    return F.feature_alpha_dropout(x, p=0.5, training=False)
