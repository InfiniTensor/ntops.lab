import torch
from ntops_lab.kernels.reduction.log_mean_exp_width64 import run

OP_NAME = "log_mean_exp_width64"
...
def run_pytorch(*inputs):
    x, = inputs
    return torch.logsumexp(x, dim=1) - torch.log(torch.tensor(float(x.shape[1]), device=x.device, dtype=x.dtype))
