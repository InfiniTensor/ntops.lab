...
def _make_conv2d_kernel(pad_h, pad_w):
    ...
    return ninetoothed.make(
        ...
        _conv2d_application,
        tensors,
        kernel_name=f"ntops_lab_conv3d_conv2d_core_{pad_h}_{pad_w}",
        max_num_configs=1,
    )
...
def run(*inputs):
    ...
    k, _, t, _, _ = w.shape
    out_d = d - t + 1
    out = torch.empty((n, k, out_d, h - 2, ww - 2), device=x.device, dtype=x.dtype)
    for od in range(out_d):
        acc = None
        for kt in range(t):
            part = _conv2d(x[:, :, od + kt, :, :], w[:, :, kt, :, :], None, padding=(0, 0))
            acc = part if acc is None else acc + part
        out[:, :, od, :, :] = acc + b[None, :, None, None]
    return out
