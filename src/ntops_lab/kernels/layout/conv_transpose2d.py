...
def _make_conv2d_kernel(pad_h, pad_w):
    ...
    return ninetoothed.make(
        ...
        _conv2d_application,
        tensors,
        kernel_name=f"ntops_lab_conv_transpose2d_conv2d_core_{pad_h}_{pad_w}",
        max_num_configs=1,
    )
...
def run(*inputs):
    x, w, b = inputs
    wf = w.permute(1, 0, 2, 3).flip(-1).flip(-2)
    return _conv2d(x, wf, b, padding=(w.shape[-2] - 1, w.shape[-1] - 1))
