...
def _make_pool2d_kernel(kind, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w):
    ...
    return ninetoothed.make(
        ...
        kernel_name=f"ntops_lab_avg_pool3d_{kind}_pool2d_{kernel_h}x{kernel_w}_s{stride_h}x{stride_w}_p{pad_h}x{pad_w}",
    )
...
def run(*inputs):
    ...
    for d in range(x.shape[2] - 1):
        acc = None
        for kd in range(2):
            part = _pool2d(x[:, :, d + kd, :, :], "avg", kernel_size=(2, 2), stride=(2, 2), padding=(0, 0))
            acc = part if acc is None else acc + part
        outs.append((acc / 2.0)[:, :, None, :, :])
