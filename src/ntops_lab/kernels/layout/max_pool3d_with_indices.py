...
def _make_pool2d_kernel(kind, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w):
    ...
    return ninetoothed.make(
        ...
        kernel_name=f"ntops_lab_max_pool3d_with_indices_{kind}_pool2d_{kernel_h}x{kernel_w}_s{stride_h}x{stride_w}_p{pad_h}x{pad_w}",
    )
...
def run(*inputs):
    (x,) = inputs
    values = []
    for d in range(x.shape[2] - 1):
        cur = None
        for kd in range(2):
            part = _pool2d(x[:, :, d + kd, :, :], "max", kernel_size=(2, 2), stride=(2, 2), padding=(0, 0))
            cur = part if cur is None else torch.maximum(cur, part)
        values.append(cur[:, :, None, :, :])
    values = torch.cat(values, dim=2)
    indices = F.max_pool3d(x, kernel_size=(2, 2, 2), stride=(1, 2, 2), padding=(0, 0, 0), return_indices=True)[1]
