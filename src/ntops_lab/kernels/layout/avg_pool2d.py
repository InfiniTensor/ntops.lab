...
def _make_pool2d_kernel(kind, kernel_h, kernel_w, stride_h, stride_w, pad_h, pad_w):
    ...
    return ninetoothed.make(
        ...
        kernel_name=f"ntops_lab_avg_pool2d_{kind}_pool2d_{kernel_h}x{kernel_w}_s{stride_h}x{stride_w}_p{pad_h}x{pad_w}",
    )
...
def run(*inputs):
    (x,) = inputs
    return _pool2d(x, "avg", kernel_size=(4, 4), stride=(2, 2), padding=(0, 0))
