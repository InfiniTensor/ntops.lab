...
def application(x, residual, weight, out, hidden):
    y = x + residual
    value = y * ntl.rsqrt(ntl.sum(y * y, axis=1) / hidden + 1.0e-5)[:, None] * weight
    out = ntl.where(value > 0.5, value - 0.5, ntl.where(value < -0.5, value + 0.5, 0.0))
...
def _kernel(hidden):
    ...
    return ninetoothed.make(
        ...
        kernel_name=f"ntops_lab_add_rms_norm_softshrink_h{hidden}", max_num_configs=1,
    )
