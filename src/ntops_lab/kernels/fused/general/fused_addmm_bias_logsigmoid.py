...
def application(c, a, b, bias, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    value = c + acc + bias
    out = (0.0 - ntl.log(1.0 + ntl.exp(0.0 - value))).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(2), Tensor(1), Tensor(2)), kernel_name="ntops_lab_fused_addmm_bias_logsigmoid")
