...
def application(c, a, b, bias, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    value = c + acc + bias
    out = (ntl.where(value > 0.5, value - 0.5, ntl.where(value < -0.5, value + 0.5, 0.0))).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(2), Tensor(1), Tensor(2)), kernel_name="ntops_lab_fused_addmm_bias_softshrink")

def run(*inputs):
    c, a, b, bias = inputs
    out = torch.empty((a.shape[0], b.shape[1]), device=a.device, dtype=a.dtype)
    kernel(c, a, b, bias, out)
    return out
