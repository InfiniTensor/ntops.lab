...
def application(a, b, bias, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    value = acc.to(ntl.float16) + bias
    out = (ntl.where(value > 0.5, value, ntl.where(value < -0.5, value, 0.0))).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(1), Tensor(2)), kernel_name="ntops_lab_fused_gemm_bias_hardshrink")
