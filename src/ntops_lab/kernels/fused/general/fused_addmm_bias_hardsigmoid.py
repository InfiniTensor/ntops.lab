...
def application(c, a, b, bias, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    value = c + acc + bias
    out = (ntl.minimum(ntl.maximum(value + 3.0, 0.0), 6.0) / 6.0).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(2), Tensor(1), Tensor(2)), kernel_name="ntops_lab_fused_addmm_bias_hardsigmoid")
