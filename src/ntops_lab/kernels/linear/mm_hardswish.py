...
def application(a, b, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    out = (acc * ntl.minimum(ntl.maximum(acc + 3.0, 0.0), 6.0) / 6.0).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(2)), kernel_name="ntops_lab_mm_hardswish")
