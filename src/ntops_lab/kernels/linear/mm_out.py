...
kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(2)), kernel_name="fg_extra_mm_out", max_num_configs=1)

def run(*inputs):
    a, b = inputs
    out = torch.empty((a.shape[0], b.shape[1]), device=inputs[0].device, dtype=inputs[0].dtype)
    kernel(*inputs, out)
    return out
