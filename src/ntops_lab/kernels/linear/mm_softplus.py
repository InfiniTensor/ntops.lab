...
def application(a, b, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    out = (ntl.log(1.0 + ntl.exp(acc))).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(2)), kernel_name="ntops_lab_mm_softplus")
