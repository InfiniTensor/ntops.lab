...
def application(a, b, bias, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    value = acc + bias
    out = (0.5 * value * (1.0 + (2.0 / (1.0 + ntl.exp(-2.0 * (0.7978845608028654 * (value + 0.044715 * value * value * value)))) - 1.0))).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(1), Tensor(2)), kernel_name="ntops_lab_gemm_bias_gelu")
