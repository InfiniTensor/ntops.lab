...
def application(a, b, out):
    ...
    for k in range(a.shape[0]):
        acc += ntl.dot(a[k], b[k])
    out = ((ntl.exp(2.0 * acc) - 1.0) / (ntl.exp(2.0 * acc) + 1.0)).to(ntl.float16)

kernel = ninetoothed.make(arrangement, application, (Tensor(2), Tensor(2), Tensor(2)), kernel_name="ntops_lab_bmm_tanh_2d")
